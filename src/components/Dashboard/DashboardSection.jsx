...
// --- TODA LA LÓGICA DE PROCESAMIENTO DE DATOS VA AQUÍ ---
...
const processDataForBarChart = (sectionEvaluations, sectionName, criteriaConfig, chartState) => {
    if (sectionName === 'Aptitudes Transversales' && chartState) {
        // ... (lógica específica para aptitudes)
    }
...
const processNonEvaluableData = (sectionEvaluations, nonEvaluableCriteria, sectionName) => {
    ...
    trackedCriteria.forEach(criterion => {
        ...
        if (criterion.trackEmptyInDashboard) {
            ...
            metrics.push({ name: `${criterion.name}/s`, type: 'select', counts: { 'Ingresados': responded, 'Sin Ingreso (N/A)': pending }});
        }
    });
    return metrics;
};
...
const DashboardSection = ({ section, evaluations, criteriaConfig, nonEvaluableCriteria, executives, executiveColorMap }) => {
    ...
    const [chartState, setChartState] = useState({ view: 'bySubsection', selectedSubsection: null });
    ...
    const barData = processDataForBarChart(sectionEvaluations, section.name, criteriaConfig, chartState);
    ...
    const overallAverage = executiveAverages.reduce((sum, exec) => sum + exec.average, 0) / executiveAverages.length || 0;
    ...
    return (
        <section className="dashboard-section">
            ...
            <div className="dashboard-grid">
                ...
                <div className="card">
                    <h4 className="card-title" style={{backgroundColor: section.color + '20', color: section.color}}>{getChartTitle()}</h4>
                    <ResponsiveContainer width="100%" height={300}>
                        <BarChart data={barData} onClick={section.name === 'Aptitudes Transversales' ? handleChartClick : null} margin={{ top: 5, right: 20, left: 20, bottom: 60 }}>
